...
class HistoryFinalAudit extends Component {
    constructor(props) {
        super(props)
        this.state = {
            list: [],
            ...
            <div>
                ...
                { this.state.list.length > 0 ? 
                    <Form className="data-form" {...itemlabel}>
                        <Row>
                            <Col span={8}>
                                <Form.Item label="终审状态">{ this.state.selectedRow.stateName }</Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item label="终审员">{ this.state.selectedRow.operName }</Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item label="终审次数">{ this.state.selectedRow.timesNum }</Form.Item>
                            </Col>
                        </Row>
                        <Row>
                            <Col span={24}>
                                <Form.Item label="终审结论" labelCol={{ span: 4 }} wrapperCol={{ span: 20 }}>{ this.state.selectedRow.summary } </Form.Item>
                            </Col>
                        </Row>
                        <Row>
                            <Col span={24}>
    ...
    renderList(){
        const coulmns = [
            { title: '终审次数', dataIndex: 'timesNum', align: 'center'},
            { title: '终审日期', dataIndex: 'operDate', align: 'center'},
            { title: '终审状态', dataIndex: 'stateName', align: 'center'},
            { title: '终审员', dataIndex: 'operName', align: 'center'}
        ]
        const props = {
            rowKey: 'id',
            columns: coulmns,
...
export default HistoryFinalAudit;