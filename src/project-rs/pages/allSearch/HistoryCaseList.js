...
import { Table, Icon, Tooltip, Modal, Tabs, Button } from 'antd';
...
import CalcBook from './CalcBook'
import CaseProgress from '../reportCase/CaseProgress'
...
class HistoryCaseList extends Component {
        ...
        return (
            <div> 
                ...
                <Modal
                    ...
                    footer={[
                        <Button key="0" onClick={ this.closeBook }>返回</Button>,
                        <Button key="1" type="primary" onClick={ this.printBook }>打印</Button>,
                    ]}
                >
                    <div className="modal-detail-content" id="calcBook" style={{ height: this.state.conHeight - 50 }}>
                        <CalcBook indemnityCaseNo={ this.state.indemnityCaseNo } />
                    </div>
                </Modal>
                <Modal
                    className="modal-detail"
            ...
            { title: '操作', key: 'action', align: 'center', width: 100,
                render: (text, record) => (
                    <div className="list-actions">
                        ...
                        <Tooltip title="赔款计算书" onClick={this.book.bind(this, record)}>
    ...
    book = record => {
        let indemnityCaseNo = record.indemnityCaseNo
        this.setState({
            bookVisible: true,
            indemnityCaseNo
        })
    ...
    printBook = () => {
        let printView = document.querySelector('#calcBook')
        let clonedNode = printView.cloneNode(true)
        document.querySelector('#root').className = 'print-hide'
        document.querySelector('.ant-modal-mask').classList.add('print-hide')
        document.querySelector('.ant-modal-wrap').classList.add('print-hide')
        document.body.appendChild(clonedNode)
        window.print()
        document.body.removeChild(clonedNode)
        document.querySelector('#root').className = ''
        document.querySelector('.ant-modal-mask').classList.remove('print-hide')
        document.querySelector('.ant-modal-wrap').classList.remove('print-hide')
    }
    closeBook = () => {
        if(document.querySelector('#calcBook')){
            document.querySelector('#calcBook').scrollTop = 0
        }
        this.setState({
            bookVisible: false,
            caseId: ''
