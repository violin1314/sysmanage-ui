        ...
        return (
            ...
            <div style={{ minHeight: 260 }}>
                <div style={{ marginBottom: 20, color: '#f00' }}>
                    {
                        sessionStorage.getItem('insuranceCode') === '1027001' ?
                        `录入就诊合规费用 = 总费用 - 社保支出 - 其他报销金额 - 乙类自理金额 - 自费金额 = ${(this.state.moneyData.medicalTotalMoney - this.state.moneyData.siCost - this.state.moneyData.otherPayMoney - this.state.moneyData.partSelfAmnt - this.state.moneyData.selfAmnt).toFixed(2) }`
                        :
                        `录入就诊合规费用 = 总费用 - 社保支出 - 其他报销金额 = ${(this.state.moneyData.medicalTotalMoney - this.state.moneyData.siCost - this.state.moneyData.otherPayMoney).toFixed(2) }`
                    }
                </div>
                    ...
                    <tbody>
                        ...
                        </tr>
                        {
                            this.state.list.map(item => (
                                <EditCalc dutys={ this.state.dutys } item={ item } key={ item.key } onDel={ this.onDel } onCalc={ this.onCalc } />
                            ))
                        }
                    </tbody>
                </table>
