...
import { Row, Col, Form, Button, Input, Select, Icon, DatePicker } from 'antd';
...
class FormSearch extends Component {
    ...
    componentDidMount() {
        utils.getCodeList(['reporter_channel', 'cancel_flag'], res => {
            this.setState({
            ...
            <Form {...formItemLayout} onSubmit={this.searchSubmit} className="form-search">
                    ...
                    <Col span={8}>
                        <Form.Item label="有效标志">
                            {getFieldDecorator('cancelFlag')(
                                <Select placeholder="请选择有效标志">
                                {
                                    this.state.cancelFlags.map(item => (
                                        <Select.Option key={ item.codeValue } value={ item.codeValue }>{ item.codeName }</Select.Option>
                                    ))
                                }
                                </Select>
                ...
                <Row type="flex" align="middle">
                    ...
                    <Col span={8} style={{ textAlign: 'right' }}>
                        ...
                        <Button type="primary" onClick={ this.add }><Icon type="plus" /> 新增报案</Button>
                    </Col>
                </Row>
            </Form>
        )
